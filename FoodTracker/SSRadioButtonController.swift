...
class SSRadioButtonsController : NSObject
{
    ...
    init(buttons: UIButton...) {
        ...
        for aButton in buttons {
            aButton.addTarget(self, action: Selector(("pressed:")), for: UIControlEvents.touchUpInside)
        }
    ...
    func removeButton(aButton: UIButton?) {
        ...
        for iteration in 0..<buttonsArray.count {
            let iteratingButton = buttonsArray[iteration]
            if iteratingButton === aButton  {
                break
            } else {
                iteratingButton.isEnabled = false
            }
            
            if iteratingButton.isEnabled == true  {
                buttonsArray.remove(at: iteration)
                iteratingButton.removeTarget(self, action: Selector(("pressed:")), for: UIControlEvents.touchUpInside)
